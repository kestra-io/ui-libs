 ...
 * @param {Array<{name:string, value:string, var:string}>} colorPalette1
 * @param {Array<{name:string, value:string, var:string}>} colorPalette2
 */
...
/**
 * get Color Value from color index
 * @param {{name:string, value:string, var:string}} color 
 * @param {Object<string, string>} colorIndex 
 * @returns {string}
 */
 ...
 * @param {Object<string, string>} colorIndex
 * @returns {{tokenScss: string, cssVariableNames: Object<string, string[]>}}
 */
...
function makePalettes(palette, paletteName, selector) {
    const baseColorNames = palette.filter(c => !c.name.startsWith("ks/"))
        .sort((a, b) => {
    ...
    const scss = baseColorNames.toSorted((a, b) => a.name.localeCompare(b.name)).map(color => `$base-${color.name.split("/").join("-")}: ${color.value};`).join("\n")
    ...
    const {tokenScss, cssVariableNames} = getVariableScss(palette.filter(c => c.name.startsWith("ks/")), colorIndex)
