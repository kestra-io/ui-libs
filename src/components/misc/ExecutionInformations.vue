...
<script lang="ts" setup>
    ...
    const taskRunList = computed(() => {
        return props.execution && props.execution.taskRunList ? props.execution.taskRunList : []
    })

    const taskRuns = computed(() => {
        return taskRunList.value.filter(t => t.taskId === Utils.afterLastDot(props.uid))
    })
    ...
    const histories = computed(() => {
        ...
        const max = Math.max(...taskRuns.value
        ...
        const duration = Math.max(...taskRuns.value
